...
#
# core.components.sprite
#
"""This module contains the player and npc sprite.
"""
import logging
import pygame
import pprint
...
class Sprite(object):
    ...
    def __init__(self, player):
        self.player = player
        self.name = self.player.name			# This is the player's name to be used in dialog
        self.sprite = {}			# The pyganim object that contains the player animations
        self.sprite_name = self.player.sprite_name # Hold on the the string so it can be sent over the network

        # Get all of the player's standing animation images.
        self.standing = {}
        standing_types = ["front", "back", "left", "right"]
        for standing_type in standing_types:
            surface = pygame.image.load(prepare.BASEDIR + 'resources/sprites/%s_%s.png' % (self.sprite_name, standing_type)).convert_alpha()
        ...
        self.inventory = self.player.inventory # The Player's inventory.
        self.monsters = self.player.monsters # This is a list of tuxemon the player has
        self.storage = self.player.storage
        self.party_limit = self.player.party_limit # The maximum number of tuxemon this player can hold 1 for testing
        self.playerWidth, self.playerHeight = self.standing["front"].get_size()    # The player's sprite size in pixels
        self.walking = self.player.walking			# Whether or not the player is walking
        self.running = self.player.running			# Whether or not the player is running
        self.moving = self.player.moving			# Whether or not the player is moving
        self.move_direction = self.player.move_direction # This is a string of the direction we're moving if we're in the middle of moving
        self.direction = self.player.direction # What direction the player is moving
        self.facing = self.player.facing	# What direction the player is facing
        self.walkrate =	self.player.walkrate # The rate in pixels per second the player is walking
        self.runrate = self.player.runrate	# The rate in pixels per second the player is running
        self.moverate = self.walkrate # The movement rate in pixels per second
        self.position = self.player.position # The player's sprite position on the screen
        self.global_pos = self.player.global_pos	# This is the offset we're going to add to the x,y coordinates of everything on the map
        self.tile_pos = self.player.tile_pos       # This is the position of the player based on tile
        self.tile_size = self.player.tile_size
        self.move_destination = self.player.move_destination # The player's destination location to move to
        self.final_move_dest = self.player.final_move_dest        # Stores the final destination sent from a client
        self.rect = pygame.Rect(self.position[0], self.position[1], self.playerWidth, self.playerHeight) # Collision rect
        self.game_variables = self.player.game_variables # Game variables for use with events
        ...
        for anim_type in anim_types:
            images_and_durations = [(prepare.BASEDIR + 'resources/sprites/%s_%s.%s.png' % (self.sprite_name, anim_type, str(num).rjust(3, '0')),
...
class NpcSprite(Sprite):
    def __init__(self, npc,  sprite_name="maple", name="Maple"):
        # Initialize the parent menu class's default shit
        Sprite.__init__(self, npc)
        self.npc = npc
        self.name = self.npc.name
        self.behavior = self.npc.behavior
        self.isplayer = self.npc.isplayer
        self.update_location = self.npc.interactions

        # These attributes are used with the new movement system
        self.walkrate = self.npc.walkrate                 # Tiles per second walk rate
        self.runrate = self.npc.runrate                  # Tiles per second run rate
        self.moverate = self.walkrate
        self.tile_destination = self.npc.tile_destination
        self.current_tile = self.npc.current_tile
        self.running = self.npc.running
        self.interactions = self.npc.interactions              # List of ways player can interact with the Npc
